...
import pymysql
import sys
from 团员管理系统.xtwh import tzz_ui
from 团员管理系统.service import service
from PyQt5.QtWidgets import *

class MyClass(tzz_ui.Ui_MainWindow):
    def __init__(self):
        super().__init__()
        self.InitUi()
        self.My_Sql()
    ...
    def InitUi(self):
        ...
        self.pushButton.clicked.connect(self.refreshquery)
    ...
    def addquery(self):
        ...
        leader = self.addleader.text()
        total = self.addtotal.text()
        new=self.addnew.text()
        quit=self.addquit.text()


        if name != "" and leader != "" and total != ""and new != ""and quit!= "":
            sql = "select 团组织名称,负责人,总人数,新增人数,转出人数 from tb_tzzcx where 团组织名称 like '%" + name + "%'"
            result = service.query2(sql)
            if len(result)>0:
                self.addname.setText("")
                QMessageBox.information(None, '提示', '您要添加的团组织已经存在，请重新输入！', QMessageBox.Ok)
            else:
                # 执行添加语句
                result = service.exec("insert into tb_tzzcx(团组织名称,负责人,总人数,新增人数,转出人数) values (%s,%s,%s,%s,%s)", (name, leader,total,new,quit))
    ...
    def deletequery(self):
        ...
        if name != "":
            ...
            result = service.query2(sql)
            if len(result) > 0:
                result2 = service.exec("delete from tb_tzzcx where 团组织名称= %s", (name,))
                ...
                QMessageBox.information(None, '提示', '您要删除的团组织不存在，请重新输入！', QMessageBox.Ok)
        else:
            QMessageBox.information(None, '提示', '请重新输入要删除的团组织名称！', QMessageBox.Ok)

    def changequery(self):
        name = self.before.text()
        if name != "":
            sql = "select 团组织名称,负责人,总人数,新增人数,转出人数 from tb_tzzcx where 团组织名称 like '%" + name + "%'"
            result = service.query2(sql)
            if len(result) > 0:
                ...
                leader=self.afterleader.text()
                total=self.aftertotal.text()
                new=self.afteradd.text()
                quit=self.addquit.text()


                if name2 != "" and leader != "" and total != ""and new != ""and quit!= "" :
                    sql = "select 团组织名称,负责人,总人数,新增人数,转出人数 from tb_tzzcx where 团组织名称 like '%" + name2 + "%'"
                    result = service.query2(sql)
                    if len(result) > 0:
                        ...
                        # 执行添加语句
                        result = service.exec(
                            "insert into tb_tzzcx(团组织名称,负责人,总人数,新增人数,转出人数) values (%s,%s,%s,%s,%s)", (name2, leader,total,new,quit))
                        if result > 0:  # 如果结果大于0，说明添加成功
                ...
                self.before.setText("")
                QMessageBox.information(None, '提示', '您要修改的团组织不存在，请重新输入！', QMessageBox.Ok)
        else:
            QMessageBox.information(None, '提示', '请重新输入要修改的团组织名称！', QMessageBox.Ok)
