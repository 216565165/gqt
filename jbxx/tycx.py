...
import pymysql
import sys
from 团员管理系统.jbxx import tycx_ui
from 团员管理系统.service import service


class MyClass(tycx_ui.Ui_Form):
    def __init__(self):
        super().__init__()
        self.InitUi()
        self.My_Sql()
    ...
    def My_Sql(self):  # 连接mysql数据库
        ...
        cur.execute('select * from tb_ty')  # 将数据从数据库中拿出来
        total = cur.fetchall()
    ...
    def query(self):
        ...
        if self.lineEdit.text()=="":
            result=service.query('select * from tb_ty')
        else:
            key = self.lineEdit.text()
            sql="select 团员姓名,团员性别,团员年龄,团员所属组织,入团时间 from tb_ty where 团员姓名 like '%" + key + "%'"
    ...
    def query2(self):
        ...
        if self.lineEdit_2.text()=="":
            ...
            sql="select 团员姓名,团员性别,团员年龄,团员所属组织,入团时间 from tb_ty where 团员所属组织 like '%" + key + "%'"
            result=service.query2(sql)
        row = len(result)  # 取得记录个数，用于设置表格的行数
        self.tableWidget.setRowCount(row)  # 设置表格行数
        self.tableWidget.setColumnCount(5)  # 设置表格列数
    ...
    def query3(self):
        ...
        if self.lineEdit_3.text()=="":
            ...
            sql="select 团员姓名,团员性别,团员年龄,团员所属组织,入团时间 from tb_ty where 团员性别 like '%" + key + "%'"
            result=service.query2(sql)
        row = len(result)  # 取得记录个数，用于设置表格的行数
        self.tableWidget.setRowCount(row)  # 设置表格行数
        self.tableWidget.setColumnCount(5)  # 设置表格列数
...
# 根据入团时间查询
    def query4(self):
        ...
        if self.lineEdit_4.text()=="":
            ...
            sql="select 团员姓名,团员性别,团员年龄,团员所属组织,入团时间 from tb_ty where 入团时间 like '%" + key + "%'"
            result=service.query2(sql)
        row = len(result)  # 取得记录个数，用于设置表格的行数
        self.tableWidget.setRowCount(row)  # 设置表格行数
        self.tableWidget.setColumnCount(5)  # 设置表格列数
